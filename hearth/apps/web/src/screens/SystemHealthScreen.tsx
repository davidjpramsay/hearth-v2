...
export function SystemHealthScreen() {
  ...
  const pendingUpdateVersion = useRef<string | null>(null);
  const updateCommandSent = useRef(false);
  ...
  const installUpdate = useMutation({
    mutationFn: async (targetVersion: string) => {
      setUpdateError(null);
      updateCommandSent.current = false;
      await authenticateWithPasskey();
      if (pendingUpdateVersion.current !== targetVersion) pendingUpdateRequestId.current = null;
      pendingUpdateVersion.current = targetVersion;
      pendingUpdateRequestId.current ??= `request_appliance_update_${crypto.randomUUID()}`;
      updateCommandSent.current = true;
    ...
    onError: (error) => {
      // Only a lost command response can mean the appliance is restarting.
      // A cancelled passkey or a rejected command must remain immediately retryable.
      setWaitingForRestart(updateCommandSent.current && !(error instanceof HearthApiError));
      setUpdateError(updateErrorMessage(error, updateCommandSent.current));
      if (error instanceof HearthApiError) pendingUpdateRequestId.current = null;
      if (updateCommandSent.current) {
    ...
    const timer = window.setInterval(() => {
      void refetchApplianceUpdate().then((result) => {
        if (result.isSuccess) {
          ...
          setWaitingForRestart(false);
          if (
            result.data.installedVersion === pendingUpdateVersion.current ||
            (result.data.operation.targetVersion === pendingUpdateVersion.current &&
              result.data.operation.phase !== 'idle')
          ) {
            setUpdateError(null);
          }
        }
    ...
    <AdminPage title="System health">
      ...
      >
        ...
        <div>
          ...
          {healthy ? null : (
            <p>
              {status.database.state !== 'ready' ? status.database.message : status.backup.message}
            </p>
          )}
        </div>
      </section>
      ...
      {applianceUpdate.data?.supported ? (
        <ApplianceUpdateCard
          error={waitingForRestart ? 'Hearth may be restarting. Reconnecting…' : updateError}
      ...
      <section className="system-connection-health" aria-labelledby="connection-health-title">
        <div className="system-section-heading">
          ...
          <Link
            ...
            data-focus-entry={
              applianceUpdate.data?.canInstall && applianceUpdate.data.updateAvailable
                ? undefined
                : 'true'
            }
...
function updateErrorMessage(error: Error, commandSent: boolean): string {
  ...
  if (commandSent) return 'Could not confirm the update. Try again.';
  if (error.name === 'NotAllowedError' || error.name === 'AbortError') {
    return 'Passkey confirmation cancelled. Try again.';
  }
  return 'Could not confirm your passkey. Try again.';
}
